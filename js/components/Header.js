	...
	render: function() {
		...
		if (this.state.isLoggedIn) {
			...
			if (this.state.user.type == 'admin') {
				menu.unshift(<li className="dropdown" key={3}><a className="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Admin <span className="caret"></span></a> <ul className="dropdown-menu"><li><Link to="/admin/orders">All Orders</Link></li><li><Link to="/admin/overview">Overview</Link></li><li><Link to="/admin/checked-in-tickets">All Checked-In Tickets</Link></li><li><Link to="/admin/check-in">Check In</Link></li><li><a onClick={this.openLookUpOrderNumberModal}>Lookup Order #</a></li><li><Link to="/admin/rooms-view">Rooms View</Link></li></ul></li>);
